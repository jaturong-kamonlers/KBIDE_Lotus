...
void LotusDevkit() {
  ...
  // MLX90614 boots in PWM mode by default — pull SCL LOW >2ms to force SMBus mode
  // (per datasheet section 8.3.3.1). Must run BEFORE Wire.begin().
  ...
  // display.init() bumps Wire to 700kHz — restore 50kHz so MLX90614 can talk SMBus
  Wire.setClock(50000);
}
...
void wait() {
  display.clear();
  display.setFont(ArialMT_Plain_16);
  display.drawString(20, 0, "     Press");
  display.drawString(20, 20, "Start Button");
  display.drawString(20, 40, "    GPIO27");
  display.display();
